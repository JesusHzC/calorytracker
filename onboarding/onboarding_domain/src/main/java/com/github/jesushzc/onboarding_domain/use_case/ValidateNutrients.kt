package com.github.jesushzc.onboarding_domain.use_case
...
class ValidateNutrients {
    ...
    operator fun invoke(
        ...
        if (carbsRatio == null || proteinRatio == null || fatRatio == null) {
            return Result.Error(
                message = UiText.DynamicString("These are not valid values")
            )
        }
        if (carbsRatio + proteinRatio + fatRatio != 100) {
            return Result.Error(
                message = UiText.DynamicString("The values must add up to 100%")
            )
        }
